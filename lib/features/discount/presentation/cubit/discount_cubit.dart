...
class DiscountCubit extends Cubit<DiscountState> {
    ...
    dio.interceptors.add(InterceptorsWrapper(
      onRequest: (options, handler) async {
        ...
        if (token != null) {
          ...
          emit(DiscountFailure('Authentication error: No valid token available', discounts: List.from(discounts)));
        }
      ...
      onError: (DioException error, handler) async {
        if (error.response?.statusCode == 401) {
          try {
            ...
            emit(DiscountFailure('Authentication error: $e', discounts: List.from(discounts)));
          }
        }
  ...
  void addDiscountToList() {
    ...
    if (days > 0 && percentage > 0) {
      ...
      emit(DiscountSuccess('Discount added to list', discounts: List.from(discounts)));
      daysRequired.clear();
  ...
  Future<void> addDiscount(String id) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        emit(DiscountFailure("Error: ${response.statusCode}", discounts: List.from(discounts)));
      }
    } catch (e) {
  ...
  void removeDiscount(int index) {
    if (index >= 0 && index < discounts.length) {
      ...
      emit(DiscountSuccess('Discount removed', discounts: List.from(discounts)));
    }
  }
}