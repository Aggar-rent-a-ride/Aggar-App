...
class ReviewCountCubit extends Cubit<ReviewCountState> {
  ...
  Future<void> getVehicleReviewsNumber(
  ...
  Future<void> getUserReviewsNumber(
      String vehicleId, String accessToken) async {
    try {
      emit(ReviewCountLoading());
      final response = await dioConsumer.get(
        "${EndPoint.getUserReviews}?userId=$vehicleId&pageNo=1&pageSize=1",
        options: Options(
          headers: {
            'Authorization': 'Bearer $accessToken',
          },
      ...
      await Future.delayed(const Duration(seconds: 2));
      emit(ReviewUserSuccess(review: response["data"]["totalPages"]));
    } catch (e) {
      emit(ReviewCountFailure(e.toString()));
    }
  }
